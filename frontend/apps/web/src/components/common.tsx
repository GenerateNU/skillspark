import type { BadgeColor, ButtonVariant } from "./types";
...
export function Badge({ children, color = "gray" }: BadgeProps) {
  ...
  return (
    <span className={`inline-flex items-center text-xs font-medium px-2 py-0.5 rounded ${styles[color]}`}>
      {children}
    </span>
  );
}
...
export function Textarea({ error, ...rest }: TextareaProps) {
  return (
    <textarea
      className={`w-full border rounded-md px-3 py-2 text-sm bg-white outline-none transition focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder:text-gray-400 resize-none ${error ? "border-red-400 bg-red-50" : "border-gray-300"}`}
      rows={3}
      {...rest}
    />
  );
...
export function Select({ error, children, ...rest }: SelectProps) {
  return (
    <select
      className={`w-full border rounded-md px-3 py-2 text-sm bg-white outline-none transition focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${error ? "border-red-400" : "border-gray-300"}`}
      {...rest}
    >
      {children}
    </select>
  ...
  return (
    <div className="relative my-5">
      ...
      <div className="relative flex justify-start">
        <span className="bg-white pr-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">{label}</span>
      </div>
    </div>
  );
}