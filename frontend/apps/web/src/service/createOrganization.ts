import {
  type CreateLocationInputBody,
  postLocation,
  type CreateOrganizationBody,
  createOrganization,
  type Organization,
  type ManagerSignUpInputBody,
  type signupManagerResponse,
  signupManager,
  getManagerByOrgId,
  type Manager,
...
export async function createOrganziationLocationAndManager({
  ...
  postalCode,
  managerInputs,
}: CreateOrganziationLocationAndManagerInput): Promise<{
  org: Organization;
  manager: Manager;
}> {
  ...
  const completeManagerInputs: ManagerSignUpInputBody[] = managerInputs.map(
    function (man: ManagerFormInput) {
      const errors = validateMgr(man);
      if (Object.keys(errors).length > 0)
        throw new Error(
          `Manager ${man.name || "unknown"} has incomplete or invalid inputs`,
        );
      return {
        name: man.name,
        email: man.email,
