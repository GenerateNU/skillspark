...
func TestLocationRepository_GetLocationByID(t *testing.T) {
	...
	assert.Equal(t, "02116", location2.ZipCode)
	assert.Equal(t, "USA", location2.Country)
	assert.Equal(t, "600 Boylston Street", location2.Address)
	...
	assert.Equal(t, "94102", location3.ZipCode)
	assert.Equal(t, "USA", location3.Country)
	assert.Equal(t, "700 Market Street", location3.Address)
}
