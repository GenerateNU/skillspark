...
func (r *ChildRepository) GetChildByID(ctx context.Context, childID uuid.UUID) (*models.Child, error) {
	...
	err = row.Scan(&child.ID, &child.Name, &child.SchoolID, &child.SchoolName, &child.BirthMonth, &child.BirthYear, &child.Interests, &child.GuardianID, &child.AvatarFace, &child.AvatarBackground, &child.CreatedAt, &child.UpdatedAt)
