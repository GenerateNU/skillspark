...
func (r *GuardianRepository) GetGuardianByUserID(ctx context.Context, id uuid.UUID) (*models.Guardian, error) {
	...
	err = row.Scan(&guardian.ID, &guardian.UserID, &guardian.Name, &guardian.Email, &guardian.Username, &guardian.ProfilePictureS3Key, &guardian.LanguagePreference, &guardian.ExpoPushToken, &guardian.CreatedAt, &guardian.UpdatedAt)
	if err != nil {
