...
func (r *EventRepository) UpdateEvent(ctx context.Context, input *models.UpdateEventDBInput, HeaderImageS3Key *string) (*models.Event, error) {
	...
	var titleEN, descEN string
	var titleTH, descTH *string

	err = row.Scan(&event.ID, &titleEN, &titleTH, &descEN, &descTH, &event.OrganizationID, &event.AgeRangeMin, &event.AgeRangeMax, &event.Category, &event.HeaderImageS3Key, &event.CreatedAt, &event.UpdatedAt)
	if err != nil {
