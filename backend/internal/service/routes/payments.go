...
func SetupPaymentRoutes(api huma.API, repo *storage.Repository, sc stripeClient.StripeClientInterface) {
	...
	huma.Register(api, huma.Operation{
		...
		Method:      http.MethodPost,
		Path:        "/api/v1/stripe/attach-pm/{guardian_id}",
		Summary:     "Attach a payment method to a guardian (testing only)",
		Description: "Directly attaches a Stripe payment method to a guardian's customer account. For development/testing only.",
		Tags:        []string{"Payments"},
	}, func(ctx context.Context, input *models.AttachPaymentMethodInput) (*models.AttachPaymentMethodOutput, error) {
		return paymentHandler.AttachPaymentMethod(ctx, input)
	})
}
